...
private:
    // the vulkan instance were all the setup information is stored
    VkInstance instance;
    // this is the messenger, a util that help to debug the program
    VkDebugUtilsMessengerEXT debugMessenger;
    // the glfw window (de to la vida)
    GLFWwindow* window;
    ...
    // this is the surface were we are going to render the resulting image (using WSI or window system integration)
    VkSurfaceKHR surface;
    // this is the swap chain, the chain that will have the images that will be uploaded to the surface,
    // has all the information like the queues, the surface to point to and the logical device
    VkSwapchainKHR swapChain;
    VkFormat swapChainImageFormat;
    VkExtent2D swapChainExtent;

    // This is the context of the pipeline, the passes that the render must do
    VkRenderPass renderPass;

    // this is for uniform variables, SUPER important!!
    ...
    // the view to the image
    std::vector<VkImageView> swapChainImageViews;

    // the frame buffer to store the images
    std::vector<VkFramebuffer> swapChainFramebuffers;

    // we need to record all the actions first in the CPU before send them to the GPU
    ...
    void recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
        ...
        // start the render pass and bind the graphic pipeline
    ...
    // when a debug message occurs, this function is called to print the message the way we want to be print
    ...
    void setupDebugMessenger() {
        ...
        Common::populateDebugMessengerCreateInfo(createInfo);
    ...
    void createGraphicsPipeline() {
        auto vertShaderCode = readFile("shadersBin/vert.spv");
        auto fragShaderCode = readFile("shadersBin/frag.spv");
        ...
        // this is to load the vertex data
        ...
        // This is for dynamic changes, thing that can be set at draw time:
        // - Scissors
        // - Viewport
        ...
        // no geometry will be pass to the rasterizer, meaning that the framebuffer is disabled.
